...
class ImportCategorias extends Command
{
    /**
     * Ejemplos:
     *  php artisan import:categorias categorias.xlsx
     *  php artisan import:categorias storage/app/imports/categorias.xlsx
     */
    protected $signature = 'import:categorias 
                            {path : Ruta del archivo .xlsx/.xls/.csv (relativa o absoluta)}';

    protected $description = 'Importa categorías desde un archivo Excel/CSV a la tabla categorias';
...
- " . storage_path('app/imports/' . ltrim($inputPath, '/\\')));
        ...
        $this->info("Importando categorías desde: {$path}");

        try {
            Excel::import(new CategoriasImport, $path);
