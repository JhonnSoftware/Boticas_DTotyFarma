...
@section('content')
                ...
                <div class="card" style="border-radius: 20px;">
                    <div class="card-body">
                        ...
                        <div class="d-flex align-items-center mb-4">
                            <h4 class="card-title" style="font-size: 24px;">Lista de Clases</h4>
                        </div>
                        <div class="d-flex justify-content-between align-items-center mb-3">
                            ...
                            <div class="d-flex flex-column flex-md-row justify-content-between align-items-stretch gap-2">
                                <div class="input-group">
                                    <form action="{{ route('clases.index') }}" method="GET" class="input-group">
                                ...
                                <button class="btnAgregarClase"
                                    ...
                                    data-bs-toggle="modal" data-bs-target="#nuevoClase">
                                    <i class="fas fa-plus"></i> Nuevo Clase
                                </button>
                            </div>
                        </div>
                                    ...
                                    @foreach ($clases as $clase)
                                        <tr>
                                            ...
                                            <td class="font-weight-medium text-dark border-top-0 px-2 py-4">
                                                <span
                                                    style="background: {{ $clase->estado == 'Activo' ? '#6ff073' : '#f06f6f' }};
                                                            color: {{ $clase->estado == 'Activo' ? '#159258' : '#780909' }};
                                            ...
                                            <td class="font-weight-medium text-dark border-top-0 px-2 py-4">
                                                ...
                                                <a href="#" class="text-success" data-bs-toggle="modal"
                                                    data-bs-target="#editarClase{{ $clase->id }}">
                                                    <i data-feather="edit"></i>
                                                </a>


                                                @if ($clase->estado === 'Activo')
                                                    <form action="{{ route('clases.desactivar', $clase->id) }}"
                                                        method="POST" style="display:inline;"
                                                        id="form-desactivar-{{ $clase->id }}">
                                                        @csrf
                                                        @method('PUT')
                                                        <a href="#" class="text-danger"
                                                            onclick="event.preventDefault(); confirmarDesactivacion({{ $clase->id }});">
                                                            <i data-feather="trash-2"></i>
                                                        </a>
                                                    </form>
                                                @else
                                                    <form action="{{ route('clases.activar', $clase->id) }}"
                                                        method="POST" style="display:inline;"
                                                        id="form-activar-{{ $clase->id }}">
                                                        @csrf
                                                        @method('PUT')
                                                        <a href="#" class="text-success"
                                                            onclick="event.preventDefault(); confirmarActivacion({{ $clase->id }});">
                                        ...
                                        <div class="modal fade" id="editarClase{{ $clase->id }}" tabindex="-1"
                                            aria-labelledby="editarClaseLabel{{ $clase->id }}" aria-hidden="true">
                                            <div class="modal-dialog modal-dialog-centered modal-lg">
                                                <div class="modal-content border-0 shadow"
                                                    ...
                                                    <!-- Formulario -->
                                                    <form action="{{ route('clases.actualizar', $clase->id) }}"
                                                        ...
                                                        <div class="modal-body px-4 py-4"
                                                            ...
                                                            <div class="row g-3">
                                                                <!-- Campo Nombre -->
                                                                <div class="col-md-12">
                                                                    <label for="nombre{{ $clase->id }}"
                                                                        class="form-label fw-semibold text-dark">Nombre</label>
                                                                    <input type="text" class="form-control rounded-3"
                                                                        id="nombre{{ $clase->id }}" name="nombre"
                                                                        value="{{ $clase->nombre }}" required>
                                                                    <div class="invalid-feedback"
                                                                        id="nombreError{{ $clase->id }}">
                                                                        Se debe ingresar el nombre de la clase.
                                                                    </div>
                                                                </div>
                                                            </div>
                                                        </div>
    ...
    <div class="modal fade" id="nuevoClase" tabindex="-1" aria-labelledby="nuevoClaseLabel"
        aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered modal-lg">
            <div class="modal-content border-0" style="border-radius: 12px; background-color: #ffffff; overflow: hidden;">
                ...
                <div class="modal-header text-white" style="background-color: #0A7ABF;">
                    <h5 class="modal-title fw-bold" id="nuevoClaseLabel">
                        <i class="bi bi-folder-plus me-2"></i>Registrar nueva clase
                    </h5>
                ...
                <div class="modal-body px-4 py-4" style="background-color: #F9F9F9;">
                    <form action="{{ route('clases.store') }}" method="POST" class="needs-validation" novalidate>
                        @csrf

                        <div class="row g-3">
                            <!-- Nombre de la clase -->
                            <div class="col-md-12">
                                <label for="nombre" class="form-label text-dark">Nombre de la clase</label>
                        ...
                        <div class="modal-footer mt-4 border-0 px-0 d-flex justify-content-end"
                            ...
                            <button type="submit" class="btn text-white rounded-3" style="background-color: #25A6D9;">
                                <i class="bi bi-save me-1"></i>Guardar clase
                            </button>
                        </div>
                    </form>
                </div>
...
@section('scripts')
    ...
    <script>
        $(document).ready(function() {
            $('#inputBusqueda').on('keyup', function() {
                ...
                $.ajax({
                    url: "{{ route('clases.buscar') }}",
                    type: "GET",
                    data: {
                        buscar: buscar
                    },
                    success: function(data) {
                        $('#tablaClases').html(data);
                        feather.replace();
                    }
                });
            });
    ...
    <script>
        function confirmarDesactivacion(id) {
            Swal.fire({
                title: '¿Estás seguro?',
                text: "La clase será marcado como inactivo",
                icon: 'warning',
        ...
        function confirmarActivacion(id) {
            Swal.fire({
                title: '¿Reingresar clase?',
                text: "La clase será marcado como activo nuevamente.",
                icon: 'question',
                showCancelButton: true,
